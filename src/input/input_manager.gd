# InputManager.gd — P1 keyboard input reader
# Reads InputMap actions and emits Events.input_action signals.
# Part of Foundation Layer (Story 001).
extends Node
class_name InputManager

## InputManager — 读取 P1 键盘输入并发射到 Events 信号总线。
## 当前仅支持 P1（键盘 WASD + J/K），P2/手柄输入由后续 Story 实现。
...
const ACTION_MOVE_LEFT := &"move_left_p1"
const ACTION_MOVE_RIGHT := &"move_right_p1"
...
func _process_p1_input() -> void:
	# Discrete actions — emit on just pressed (one-shot)
	if Input.is_action_just_pressed(ACTION_JUMP):
		Events.input_action.emit(PLAYER_ID_P1, ACTION_JUMP, 1.0)

	if Input.is_action_just_pressed(ACTION_DODGE):
		Events.input_action.emit(PLAYER_ID_P1, ACTION_DODGE, 1.0)

	if Input.is_action_just_pressed(ACTION_ATTACK_LIGHT):
		Events.input_action.emit(PLAYER_ID_P1, ACTION_ATTACK_LIGHT, 1.0)

	if Input.is_action_just_pressed(ACTION_ATTACK_HEAVY):
		Events.input_action.emit(PLAYER_ID_P1, ACTION_ATTACK_HEAVY, 1.0)
	...
	if net_move != 0.0:
		Events.input_action.emit(PLAYER_ID_P1, ACTION_MOVE_HORIZONTAL, net_move)
